...
def rdmcplot(Y, X, C, nbinsmat=None, binselectvec=None, scalevec=None,
             ...
             covs_dropvec=None, masspointsvec=None, ci=None, shade=False,
             col_bins=None, pch_bins=None,
             col_poly=None, lty_poly=None, col_xline=None, lty_xline=None,
             nobins=False, nopoly=False, noxline=False, nodraw=False,
             subset=None, data=None):
    
    """
    Data-driven RD plots for multi-cutoff designs

    rdmcplot() implements data-driven Regression Discontinuity (RD) plots for
    designs with multiple cutoffs.
    ...
    Parameters:
        ...
        masspointsvec: Vector indicating how to handle repeated values at each cutoff. See rdplot() for details.
        ci: Adds confidence intervals of the specified level to the plot. See rdrobust() for details.
        shade: Passes shaded confidence interval option to rdplot().
    ...
    C = np.asarray(_resolve_data_arg(C, data))
    ...
    for c in clist:
        cutoff_mask = (C == c) & (X <= c + haux[count, 1]) & (X >= c - haux[count, 0])
        yc = Y[cutoff_mask]
        xc = X[cutoff_mask]
        dc = D[cutoff_mask]
        yc0 = yc[dc == 0]
        ...
        if weightsvec is not None and len(weightsvec) == len(Y):
            weightsc = weightsvec[cutoff_mask, count]
        else:
            weightsc = weightsvec[count]
        ...
        try:
            ...
            if ci is not None:
                ci_l = aux.vars_bins.iloc[:, 7].to_numpy()
                ci_r = aux.vars_bins.iloc[:, 8].to_numpy()
    ...
    if nodraw:
        rdmc_plot = Figure().add_subplot(111)
    else:
    ...
    if ci is not None:
        for c in range(cnum):
            color = col_bins[c % len(col_bins)]
            yerr = [
                Ymean.iloc[:, c].values - CI_l.iloc[:, c].values,
                CI_r.iloc[:, c].values - Ymean.iloc[:, c].values
            ]
    ...
    if not nopoly:
        for c in range(cnum):
            color = col_poly[c % len(col_poly)]
            rdmc_plot.plot(X0.iloc[:, c].values,
                            Yhat0.iloc[:, c].values,
                            color=color,
                            linestyle=lty_poly[c % len(lty_poly)])
    ...
    if not noxline:
        for c in range(cnum):
            rdmc_plot.axvline(x=clist[c],
                              color=col_xline[c % len(col_xline)],
                              linestyle=lty_xline[c % len(lty_xline)])
