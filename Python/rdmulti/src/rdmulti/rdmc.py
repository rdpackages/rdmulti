...
def rdmc(Y,X,C,fuzzy=None,derivvec=None,pooled_opt=None,verbose=False,
    ...
    rdmc() implements point estimation and robust bias-corrected inference for
    ...
    - sharpbwvec: vector indicating whether fuzzy RD bandwidth selection uses the sharp RD model at each cutoff.
    - allvec: vector indicating whether rdrobust() reports all inference procedures at each cutoff.
    ...
    if weightsvec is not None:
        weightsvec = np.asarray(weightsvec).reshape(len(Y),-1)
    ...
    if sharpbwvec is None: sharpbwvec = np.repeat(False,cnum)
    if allvec is None: allvec = np.repeat(None,cnum)
    if covs_mat is not None:
    ...
    for c in clist:
        ...
        cutoff_mask = np.abs(C-c)<=epsilon
        yc = Y[cutoff_mask]
        xc = Xc[cutoff_mask]
    ...
    B[0,cnum] = np.dot(Baux[0,:cnum], W[0,:])
    V[0,cnum] = np.dot(Vaux[0,:cnum], W[0,:]**2)
    Coefs[0,cnum] = np.dot(Coefsaux[0,:cnum], W[0,:])
    V_cl[0,cnum] = np.dot(Vaux_cl[0,:cnum], W[0,:]**2)
