...
def _resolve_data_arg(value, data):
    if data is not None and isinstance(value, str):
        return data[value]
    return value

def _resolve_cutoffs_arg(value, data):
    if data is not None and isinstance(value, str):
        return pd.unique(pd.Series(data[value]).dropna())
    return value

def _resolve_data_matrix(value, data):
    if data is not None:
...
def rdms(Y, X, C, X2=None, zvar=None, C2=None, rangemat=None, xnorm=None,
         ...
         sharpbwvec=None, allvec=None, level=95, plot=False,
         conventional=False, subset=None, data=None):
    
    """
    Point estimation and robust bias-corrected inference for multi-score designs

    rdms() implements point estimation and robust bias-corrected inference for
    Regression Discontinuity (RD) designs with multiple scores, including
    ...
    Parameters:
        ...
        sharpbwvec: Vector indicating whether fuzzy RD bandwidth selection uses the sharp RD model at each cutoff. See rdrobust() for details.
        allvec: Vector indicating whether rdrobust() reports all inference procedures at each cutoff. See rdrobust() for details.
        ...
        subset: Optional subset of observations to use.
        data: Optional pandas DataFrame for resolving string variable names.
    ...
    Y = np.asarray(_resolve_data_arg(Y, data))
    X = np.asarray(_resolve_data_arg(X, data))
    C = np.asarray(_resolve_cutoffs_arg(C, data), dtype=float)
    if C2 is not None:
        C2 = np.asarray(_resolve_cutoffs_arg(C2, data), dtype=float)
    X2 = _resolve_data_arg(X2, data)
    zvar = _resolve_data_arg(zvar, data)
    xnorm = _resolve_data_arg(xnorm, data)
    fuzzy = _resolve_data_arg(fuzzy, data)
    cluster = _resolve_data_arg(cluster, data)
    covs_mat = _resolve_data_matrix(covs_mat, data)
    weightsvec = _resolve_data_matrix(weightsvec, data)
    n_orig = len(Y)

    if subset is not None:
        subset = np.asarray(_resolve_data_arg(subset, data))
        Y = Y[subset]
        X = X[subset]
        if X2 is not None:
            X2 = np.asarray(X2)[subset]
        if zvar is not None:
            zvar = np.asarray(zvar)[subset]
        if xnorm is not None:
            xnorm = np.asarray(xnorm)[subset]
        if fuzzy is not None:
            fuzzy = np.asarray(fuzzy)[subset]
        if cluster is not None:
            cluster = np.asarray(cluster)[subset]
        if covs_mat is not None:
            covs_mat = np.asarray(covs_mat)[subset, :]
        if weightsvec is not None and not isinstance(weightsvec, str):
            weights_arr = np.asarray(weightsvec)
            if weights_arr.shape[0] == n_orig:
                weightsvec = weights_arr[subset]

    if X2 is not None:
        X2 = np.asarray(X2)
    if zvar is not None:
        zvar = np.asarray(zvar)
    if xnorm is not None:
        xnorm = np.asarray(xnorm)
    if fuzzy is not None:
        fuzzy = np.asarray(fuzzy)
    if cluster is not None:
    ...
    if bmat is not None:
        if np.isscalar(bmat): bmat = np.full((cnum, 2), bmat)
    ...
    if weightsvec is not None:
        weightsvec = np.asarray(weightsvec)
        if weightsvec.ndim == 1:
            weightsvec = np.tile(weightsvec.reshape(-1, 1), (1, cnum))
    ...
    if sharpbwvec is None:
        sharpbwvec = np.full(cnum, False)
    if allvec is None:
        allvec = np.full(cnum, None)
    if covs_mat is not None:
    ...
    if X2 is None:
        ...
        for c in range(cnum):
            xc_all = X - C[c]
            cutoff_mask = (xc_all >= Rc[c, 0]) & (xc_all <= Rc[c, 1])
            yc = Y[cutoff_mask]
            xc = xc_all[cutoff_mask]

            weightsc = None
            if weightsvec is not None:
                weightsc = weightsvec[cutoff_mask, c]

            covs_aux = None
            if covs_mat is not None:
                covs_mat_c = covs_mat[cutoff_mask, :]
                if covs_list is not None:
                    covs_aux = covs_mat_c[:, covs_list[c]]
                else:
                    covs_aux = covs_mat_c

            fuzzyc = fuzzy[cutoff_mask] if fuzzy is not None else None

            if cluster is not None:
                cc  = cluster[cutoff_mask]
            else:
                cc = None
        ...
        for c in range(cnum):
            xc_all = np.sqrt((X - C[c])**2 + (X2 - C2[c])**2) * (2 * zvar - 1)
            cutoff_mask = (xc_all >= rangemat[c, 0]) & (xc_all <= rangemat[c, 1])
